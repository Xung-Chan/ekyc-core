...
class CardScannerFrameProcessorPlugin(
    ...
    override fun callback(frame: Frame, arguments: Map<String, Any>?): Any? {
        ...
        if (now - lastProcessedTimestamp < params.throttleMs) {
            return null
        }
        ...
        try {
            ...
            if (isDoc) {
                Log.i(
                    "EkycCardScanner",
                    "Frame validation: blurVal=$blurVal (threshold=${params.blurThreshold}), glarePct=$glarePct (threshold=${params.glareThreshold}), canStartOcr=$canStartOcr, passedAll=$passedAllThresholds"
                )
            }
            ...
            if (passedAllThresholds && canStartOcr) {
                Log.i(
                    "EkycCardScanner",
                    "passedAllThresholds is TRUE, scheduling saveAndCropCardAsync for frame $frameIndex"
                )
                ...
                manager.saveAndCropCardAsync(
                    ...
                    bufferOrientation = params.bufferOrientation,
                    blurVal = blurVal,
                    glarePct = glarePct,
        ...
        } else if (blurVal < params.blurThreshold) {
            errCode = "IMAGE_TOO_BLURRY"
            errMsg = "Hình ảnh bị mờ, vui lòng giữ yên thiết bị"
        } else if (glarePct > params.glareThreshold) {
